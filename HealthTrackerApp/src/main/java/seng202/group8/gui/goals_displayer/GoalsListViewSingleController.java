...
public class GoalsListViewSingleController {
    ...
    /**
     * The function to set all of the fxml labels and button actions for a single goal in the lost view
    ...
    public void start() {
        switch (GoalType.fromEnumToString(currentGoal.getGoalType())) {
            case "Activity":
                ...
                type.setText(DataType.fromEnumToString(currentGoal.getDataType()));
                type.setOpacity(1);
                break;
            case "Frequency":
                currentGoal.calculateCurrent();
